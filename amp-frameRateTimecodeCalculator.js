        ...
        player.ready(function () {
            ...
            player.addEventListener(amp.eventName.loadedmetadata, function () {
                ...
                if (streamingUrlComponentIndex !== -1) {
                    var dashManifestUrl;  //willzhan change: if currentSrc is a DASH URL under RPv3, use it directly
                    if (currentSrc.lastIndexOf("manifest(format=mpd-time-") > 0){
                        dashManifestUrl = currentSrc;
                    } else {
                        dashManifestUrl = currentSrc.substring(0, streamingUrlComponentIndex + streamingUrlComponent.length) + '(format=mpd-time-csf)';
                    }
